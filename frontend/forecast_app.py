...
if st.button("View Transaction Summary"):
    try:
        ...
        # show raw response for debugging (remove/comment out later)
        # st.write("DEBUG raw response:", data_json)
        ...
        if not records:
            ...
            if not amount_field:
                ...
                if "date" in df.columns and df["date"].notna().any():
                    trend = df.dropna(subset=["date"]).sort_values("date").set_index("date").resample("D")["amount"].sum().rolling(7).mean()
                    st.line_chart(trend)
